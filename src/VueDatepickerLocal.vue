...
<div class="datepicker" :class="{'datepicker-range':range,'datepicker__clearable':clearable&&text&&!disabled}">
  ...
  <input @change="dateChange" :value="text" :disabled="disabled" :placeholder="placeholder" :name="name">
...
export default {
  ...
  watch: {
    value () {
      console.log("inside watcher"+this.value)
  ...
  methods: {
    dateChange (e){
      ...
      this.$emit('date-change',userdate)
      console.log("userip"+e.target.value)
    },
    cls () {
